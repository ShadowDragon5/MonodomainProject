...
def train_monodomain(model, epochs=5000, lr=1e-3, N_f=10000, N_ic=1000, N_bc=1000):
    ...
    branch_input_f = torch.tensor([[Sigma_h, Sigma_h, Sigma_h]], device=device).repeat(
        N_f, 1
    )
    ...
    branch_input_ic = torch.tensor([[Sigma_h, Sigma_h, Sigma_h]], device=device).repeat(
        N_ic, 1
    )
    ...
    branch_input_bc = torch.tensor([[Sigma_h, Sigma_h, Sigma_h]], device=device).repeat(
        N_bc, 1
    )
    ...
    for ep in range(epochs):
        ...
        if (ep) % 500 == 0:
            print(
                f"Epoch {ep + 1:5d} | total-loss {loss.item():.3e} (PDE: {loss_pde.item():.3e}, IC: {loss_ic.item():.3e}, BC: {loss_bc.item():.3e})"
            )
